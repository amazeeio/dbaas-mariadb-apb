...
if true || [[ -z "$KUBECONFIG" ]] || [[ $(realpath "$KUBECONFIG") = $(realpath "$HOME/.kube/config") ]]; then
...
updatedBrokerConfig=$(
	...
	jq '. += {secrets: [{title: "DBaaS database credentials", secret: "dbaas-db-credentials", apb_name: "localregistry-dbaas-mariadb-apb"}]}' |
